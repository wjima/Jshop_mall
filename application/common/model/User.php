...
//            }
    ...
    public function manageAdd($data)
    {
        ...
        if ($result) {
            $data['nickname'] = $data['nickname']?$data['nickname']:'用户';
            sendMessage($this->id, 'new_user_notice', $newData);
        ...
        switch ($source) {
            case 1:
                ...
                if ($type == 1) {
                    ...
                    //商品
                    if ($user_id) {
                        $qrc_data = $return_url . '?scene=id%253D' . $id . 'invite%253D' . $team_id . 'group_id%253D' . $group_id;
                    } else {
