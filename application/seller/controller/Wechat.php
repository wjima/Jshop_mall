...
class Wechat extends Seller
{
    ...
    {
        if(getSellerInfoById($this->sellerId,'store_type'))
        {
            //共享店铺
            $token = getSellerInfoById($this->sellerId, 'token');
            $filename = "qrcode/".$token.'-430--0-0-0-.jpg';
            if(file_exists($filename))
            {
                $this->assign('img', $filename);
            }

            return $this->fetch('info-share');
        }
        else
        {
            //独立店铺
    ...
    {
        $wx = new Wx();
        $access_token = $wx->getAccessToken(config('jshop.wxapp.appid'), config('jshop.wxapp.appsecret'));
        $token = getSellerInfoById($this->sellerId, 'token');

        $is_hyaline = input('is_hyaline', 'false') == 'false'?false:true;
        $style = [
            'width' => input('width', 430),
            'auto_color' => input('auto_color', false),
            'line_color_r' => input('line_color_r', 0),
            'line_color_g' => input('line_color_g', 0),
            'line_color_b' => input('line_color_b', 0),
            'is_hyaline' => $is_hyaline
        ];
        $res = $wx->getParameterQRCode($access_token, $token, config('jshop.wx_pages'), $style);
        return $res;
    }
