		...
		<view class='cell-group margin-cell-group'>
			...
			<view v-if="type == 1">
				<view class='cell-item flex-item'>
					...
					<view class='cell-item-ft' v-for="(item,index) in orderInfo.rel" :key="index">
						<view class="cell-ft-p" @click="orderDetail(item.source_id)">{{ item.source_id || ''}}</view>
						<!-- <view class="cell-ft-p" @click="orderDetail(orderId)">{{ orderId }}</view> -->
					</view>
				</view>
				<view class='cell-item'>
					<view class='cell-item-hd'>
						<view class='cell-hd-title'>支付金额</view>
					</view>
					<view class='cell-item-ft'>
