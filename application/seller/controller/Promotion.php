...
class Promotion extends Seller
{
    ...
    public function conditionList()
    {
        ...
        //校验是否有此权限
        $promotionModel = new PromotionModel();
        $pwhere['seller_id'] = $this->sellerId;
    ...
    public function conditionEdit()
    {
        ...
        if(input('?param.id')){
            $info = $conditionModel->getInfo(input('param.id'),$this->sellerId);
            if(!$info){
    ...
    public function conditionDel()
    {
        ...
        $conditionModel = new PromotionCondition();
        return $conditionModel->toDel(input('param.id'),$this->sellerId);
    }
    ...
    public function resultEdit()
    {
        ...
        //如果是修改，就取数据，否则就是新增，直接渲染模板
        if(input('?param.id')){
            $info = $resultModel->getInfo(input('param.id'),$this->sellerId);
            if(!$info){
                return error_code(15004);
            }
            $code = $info['code'];
    ...
    public function resultDel()
    {
        ...
        $resultModel = new PromotionResult();
        return $resultModel->toDel(input('param.id'),$this->sellerId);
    }
