<template>
  <div class="cashierdesk">
    ...
    <yd-popup class="payresult" v-model="popShow" position="center" width="70%" :close-on-masker="false">
      <div>
...
export default {
  ...
  methods: {
      ...
      if (code === 'wechatpay') {
        ...
        if (this.type == 1 && this.ids) {
          data['params'] = {
            trade_type: 'WAP',
            return_url:
              this.GLOBAL.locationHost() + '/#/payresult?order_id=' + this.ids
          }
        } else if (this.type == 2 && this.money) {
          data['params'] = {
            money: this.money,
    ...
    heavyLoad() {
      ...
      this.$router.replace({
        path: '/cashierdesk',
        query: { ids: this.ids, type: this.type }
      })
    }
  },
  watch: {
