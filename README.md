...
1.1. 用户等级升级插件
1.2. 库存管理插件
1.3. 插件菜单配置文件优化（https://www.jihainet.com/index/article/index/id/116.html）
1.4. 节点权限功能完善插件刷新功能
2. 用户：
2.1. 优化用户列表筛选
2.2. 新增备注字段
...
3.4. 商品列表sql注入漏洞修复
3.5. 商品详情加库存信息
3.6. 关闭规格时，商品中自定义规格值置为空
3.7. 商品类型bug修复
3.8. 商品编辑
3.9. 扩展分类没有选择的不保存
4. 订单：
4.1. 计划任务取消订单把拼团，秒杀，团购也加进去了
4.2. 订单赠品评论优化，赠品就不让评价了
4.3. 修复订单定时签收
4.4. 订单编辑订单详情显示下单人
4.5. 订单促销的时候，订单表上的商品金额优化
4.6. 优化修改订单价格和邮费
4.7. 防止重复创建订单
5. 促销：
5.1. 促销重构
5.2. 赠品促销库存判断的bug
5.3. 购物车计算促销应用之前的商品优惠
5.4. 修复没有促销类型时报错
5.5. 新增促销第X件减X钱
5.6. 促销条件一条都没有的时候，促销不生效
5.7. 促销编辑的时候，增加说明文字
5.8. 促销条件满足的时候才排他
5.9. 促销排他修复 
5.10. 砍价优化 拼团优化 团购秒杀优化
5.11. 修复多个促销条件时，团购秒杀问题
6. 团购：
6.1. 团购秒杀优化
6.2. 团购秒杀可视化布局优化
6.3. 团购秒杀添加商品修复未加类型判断
6.4. 增加拼团团购海报
6.5. 限购数量优化
6.6. 团购秒杀促销结果屏蔽
6.7. 团购秒杀详情页销量，海报价格问题修复
6.8. 修复编辑团购秒杀删除条件后token验证出错问题
6.9. 团购秒杀都增加上下架过滤
6.10. 团购首页列表价格修复
6.11. 优化团购秒杀切换规格的划线价
7. 秒杀：团购秒杀优化sql
...
8.2. 拼团销量统计，首页价格修复
8.3. 拼团详情页优化
8.4. 拼团，参团bug修复，拼团记录倒排序优化
8.5. 拼团详情页复制链接
8.6. 拼团商品删除时，不让进入拼团详情页
8.7. 决拼团分享之后不能直接参团
9. 海报：
9.1. 海报字体换为阿里巴巴普惠字体
9.2. 海报URL修复
...
9.4. 首页分享海报制作
10. 优惠券：
10.1. 修复优惠券问题删除优惠券数据的时候把用户未使用的优惠券一并删掉
11. 其他：
11.1. 微信小程序直播
11.2. 绑定手机号
11.3. 样式优化
...
11.5. 修改导入导出日志内容类型
11.6. 前端个人信息添加用户名手机号
11.7. 优化后台token验证机制，默认开启防CSRF攻击以及防止重复提交表单，如果不想开启，config\app.php里面validate_token改为false即可
